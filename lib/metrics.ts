...
export function calculateTier1Metrics(student: any, activity: any): Metrics {
  ...
  // Normalización temporal (Auditoría Técnica)
  ...
  // Detección de Estancamiento (Work Stall) [cite: 83, 86, 102]
  ...
  const velocityScore = (student?.schedule?.monGoal || 0) > 0 ? Math.min(Math.round(((totals.xpAwarded || 0) / (student.schedule.monGoal * 5)) * 100), 100) : 0;
