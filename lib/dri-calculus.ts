...
export function calculateDRIMetrics(student: Student) {
  ...
  // PDI: Decaimiento de Precisión (Fatiga)
  ...
  else if (precisionDecay > 1.4 || student.metrics?.ksi < 60) driTier = 'YELLOW';

  return { iROI: parseFloat(((student.activity?.xpAwarded || 0) / (student.activity?.time || 1)).toFixed(2)), debtExposure, precisionDecay, driTier, driSignal: driTier === 'RED' ? 'Critical Debt' : driTier === 'YELLOW' ? 'Stability Risk' : 'Flowing' };
}
