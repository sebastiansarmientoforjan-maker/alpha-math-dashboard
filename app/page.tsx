        ...
        {viewMode === 'TRIAGE' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
            {[
                ...
                { label: '⚡ Honors Track', data: greenZone, tier: 'GREEN', color: 'text-emerald-500', border: 'border-emerald-500' }
            ].map(col => (
              <div key={col.tier} className="flex flex-col bg-slate-900/20 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
                <div className={`p-4 bg-slate-900/40 border-b border-slate-800 font-black text-[10px] uppercase tracking-widest flex justify-between ${col.color}`}>
                  <span>{col.label}</span>
                  <span className="bg-slate-800 text-slate-500 px-2 py-0.5 rounded">{col.data.length} UNITS</span>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                  {col.data.map(s => (
                    <div key={s.id} onClick={() => setSelectedStudent(s)} className={`p-4 bg-slate-900/80 rounded-2xl border-l-4 ${col.border} cursor-pointer hover:scale-[1.02] transition-all`}>
